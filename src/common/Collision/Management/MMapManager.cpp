...
namespace MMAP
{
    ...
    bool MMapManager::loadMapData(uint32 mapId)
    {
        ...
        if (!file)
        {
            ...
            sWorld.outDebug(LOG_FILTER_MAPS, "MMAP:loadMapData: Error: Could not open mmap file '%s'", fileName);
#endif
            delete [] fileName;
            return false;
        }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        sWorld.outDetail("MMAP:loadMapData: Loaded %03i.mmap", mapId);
#endif
    ...
    //    if (!map)
    //    {
    //        //sLog.outLog("ZOMG! MoveMaps: BaseMap not found!");
    ...
    bool MMapManager::loadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            sWorld.outDebug(LOG_FILTER_MAPS, "MMAP:loadMap: Could not open mmtile file '%s'", fileName);
#endif
            delete [] fileName;
            return false;
        }
        ...
        if (stat == DT_SUCCESS)
        {
            ...
            sWorld.outDetail("MMAP:loadMap: Loaded mmtile %03i[%02i,%02i] into %03i[%02i,%02i]", mapId, x, y, mapId, header->x, header->y);
#endif
            return true;
        }
        else
    ...
    bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y)
    {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            sWorld.outDebug(LOG_FILTER_MAPS, "MMAP:unloadMap: Asked to unload not loaded navmesh map. %03u%02i%02i.mmtile", mapId, x, y);
#endif
            return false;
        }
        ...
        if (mmap->mmapLoadedTiles.find(packedGridPos) == mmap->mmapLoadedTiles.end())
        {
            // file may not exist, therefore not loaded
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            sWorld.outDebug(LOG_FILTER_MAPS, "MMAP:unloadMap: Asked to unload not loaded navmesh tile. %03u%02i%02i.mmtile", mapId, x, y);
#endif
            return false;
        }
        ...
        else
        {
            ...
            --loadedTiles;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            sWorld.outDetail("MMAP:unloadMap: Unloaded mmtile %03i[%02i,%02i] from %03i", mapId, x, y, mapId);
#endif
            return true;
        }
    ...
    bool MMapManager::unloadMap(uint32 mapId)
    {
        ...
        if (loadedMMaps.find(mapId) == loadedMMaps.end())
        {
            // file may not exist, therefore not loaded
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            sWorld.outDebug(LOG_FILTER_MAPS, "MMAP:unloadMap: Asked to unload not loaded navmesh map %03u", mapId);
#endif
            return false;
        }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        sWorld.outDetail("MMAP:unloadMap: Unloaded %03i.mmap", mapId);
#endif
    ...
    bool MMapManager::unloadMapInstance(uint32 mapId, uint32 instanceId)
    {
        ...
        if (loadedMMaps.find(mapId) == loadedMMaps.end())
        {
            // file may not exist, therefore not loaded
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            sWorld.outDebug(LOG_FILTER_MAPS, "MMAP:unloadMapInstance: Asked to unload not loaded navmesh map %03u", mapId);
#endif
            return false;
        }
        ...
        {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            sWorld.outDebug(LOG_FILTER_MAPS, "MMAP:unloadMapInstance: Asked to unload not loaded dtNavMeshQuery mapId %03u instanceId %u", mapId, instanceId);
#endif
            return false;
        }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        sWorld.outDetail("MMAP:unloadMapInstance: Unloaded mapId %03u instanceId %u", mapId, instanceId);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            sWorld.outDetail("MMAP:GetNavMeshQuery: created dtNavMeshQuery for mapId %03u instanceId %u", mapId, instanceId);
#endif
