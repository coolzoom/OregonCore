...
namespace VMAP
{
    ...
    class AreaInfoCallback
    {
        public:
            ...
            void operator()(const Vector3& point, uint32 entry)
            {
                ...
                sWorld.outDebug(LOG_FILTER_MAPS, "AreaInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    class LocationInfoCallback
    {
        public:
            ...
            {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS) && defined(VMAP_DEBUG)
                sWorld.outDebug(LOG_FILTER_MAPS, "LocationInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
                ...
                if (result)
                {
                    ...
                    if (fread(&referencedVal, sizeof(uint32), 1, tf) == 1)
                    {
                        if (!iLoadedSpawns.count(referencedVal))
                        {
                            ...
                            {
                                sWorld.outDebug(LOG_FILTER_MAPS, "StaticMapTree::LoadMapTile() : invalid tree element (%u/%u)", referencedVal, iNTreeValues);
                                continue;
                            }
#endif
                        ...
                        else
                        {
                            ...
                            if (iTreeValues[referencedVal].ID != spawn.ID)
                                sWorld.outDebug(LOG_FILTER_MAPS, "StaticMapTree::LoadMapTile() : trying to load wrong spawn in node");
                            else if (iTreeValues[referencedVal].name != spawn.name)
                                sWorld.outDebug(LOG_FILTER_MAPS, "StaticMapTree::LoadMapTile() : name collision on GUID=%u", spawn.ID);
#endif
                        }
                    }
                    else
