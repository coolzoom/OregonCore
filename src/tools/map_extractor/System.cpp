...
{
    char mpqFileName[1024];
    char outputFileName[1024];
    char mpqMapName[1024];
    ...
    for(uint32 z = 0; z < map_count; ++z)
    {
        ...
        sprintf(mpqMapName, "World\\Maps\\%s\\%s.wdt", map_ids[z].name, map_ids[z].name);
        WDT_file wdt;
        ...
        for(uint32 y = 0; y < WDT_MAP_SIZE; ++y)
        {
            for(uint32 x = 0; x < WDT_MAP_SIZE; ++x)
            {
                if (!wdt.main->adt_list[y][x].exist)
                    continue;
                sprintf(mpqFileName, "World\\Maps\\%s\\%s_%u_%u.adt", map_ids[z].name, map_ids[z].name, x, y);
                sprintf(outputFileName, "%s/maps/%03u%02u%02u.map", output_path, map_ids[z].id, y, x);
